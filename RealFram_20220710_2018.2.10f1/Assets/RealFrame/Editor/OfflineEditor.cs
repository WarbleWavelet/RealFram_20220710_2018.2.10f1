...
public class OfflineEditor 
{
    ...
    [MenuItem(DefinePath.MenuItem_Offline+ "Create OfflineData", false, 100)]
  ...
  [MenuItem(DefinePath.MenuItem_Offline + "Create UIOfflineData", false, 101)]
    ...
    [MenuItem(DefinePath.MenuItem_Offline + "Create UIOfflineData All", false, 102)]
    static void AssetsCreateAllUIOfflineData()
    {
        string[] guidArr = AssetDatabase.FindAssets(DefinePath_UIOfflineData.m_Type, new string[] { DefinePath_UIOfflineData.m_Path });
    ...
    [MenuItem(DefinePath.MenuItem_Offline + "Reset UIOfflineData All", false, 103)]
    static void AssetsResetAllUIOfflineData()
    {

        string[] guidArr = AssetDatabase.FindAssets(DefinePath_OfflineData.m_Type, new string[] { DefinePath_OfflineData.m_Path });
        for (int i = 0; i < guidArr.Length; i++)
        {
           string path= AssetDatabase.GUIDToAssetPath(guidArr[i]);
            GameObject go=AssetDatabase.LoadAssetAtPath<GameObject>(path);
 ...
 [MenuItem(DefinePath.MenuItem_Offline + "Create ParticleOfflineData All", false, 104)]
    static void AssetsCreateAllParticleOfflineData()
    {

        string[] guidArr = AssetDatabase.FindAssets(DefinePath_ParticleOfflineData.m_Type, new string[] { DefinePath_ParticleOfflineData.m_Path });
        for (int i = 0; i < guidArr.Length; i++)
        {
            string path = AssetDatabase.GUIDToAssetPath(guidArr[i]);
            GameObject go = AssetDatabase.LoadAssetAtPath<GameObject>(path);
